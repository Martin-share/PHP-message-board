...
if (in_array(($action = get('action')), ['edit', 'delete'])) {
    ...
    if ($password === '') {
        $update = $DB->query($sql = sprintf(
            "UPDATE users SET nickname='%s', sex=%d, user_right=%d ,qq=%d, email='%s' WHERE uid=%d",
            $nickname,
            $sex,
            $user_right,
            $qq,
        ...
        $update = $DB->query($sql = sprintf(
            "UPDATE users SET nickname='%s', password='%s', sex=%d, user_right=%d ,qq=%d, email='%s' WHERE uid=%d",
            $nickname,
            $password,
            $sex,
            $user_right,
...
if ($isLogin) {
    if (intval($uINFO['user_right']) === 1) {
        $sql = sprintf(
            "SELECT uid+10000 AS uid,nickname,sex,email,qq,reg_ip,reg_time,user_right FROM users ORDER BY uid DESC LIMIT %d,%d",
            (intval($page) - 1) * 10,
            10
        );
        $res = $DB->query($sql);
                ...
                <tbody>
                    ...
                    <?php foreach ($row as $v) : ?>
                        <tr data-uid="<?php echo $v['uid']; ?>">
                            ...
                            <td>
                                <a data-toggle="modal" data-target="#editUser" data-uid="<?php echo $v['uid']; ?>" data-nickname="<?php echo $v['nickname']; ?>" data-password="" data-sex="<?php echo $v['sex']; ?>" data-user_right="<?php echo $v['user_right']; ?>" data-qq="<?php echo $v['qq']; ?>" data-email="<?php echo $v['email']; ?>" href="javascript:;">编辑</a>
