...
if (in_array(($action = get('action')), ['edit', 'delete'])) {
    ...
    if ($action === 'delete') {
        ...
        $res = $DB->query(sprintf("CALL DeleteUserData(%d);", $uid));
        if ($res) {
            die(echoApiData(0, "删除用户成功！"));
        }
        # die(var_dump(sprintf("DELETE FROM users WHERE uid=%d", $uid)));
    ...
    //所以不考虑 SQL注入问题
    if ($password === '') {
        $update = $DB->query($sql = sprintf(
            "UPDATE users SET nickname='%s', sex=%d, qq=%d, email='%s' WHERE uid=%d",
            $nickname,
            $sex,
            $qq,
            $email,
        ...
        $password = strtolower(md5($password . $res['reg_ip']));
        // die(echoApiData(-1, $password));
        $update = $DB->query($sql = sprintf(
            "UPDATE users SET nickname='%s', password='%s', sex=%d, qq=%d, email='%s' WHERE uid=%d",
            $nickname,
            $password,
            $sex,
            $qq,
...
if ($isLogin) {
    if ($isLoginAdmin) {
        $sql = sprintf(
            "SELECT uid+10000 AS uid,nickname,sex,email,qq,reg_ip,reg_time FROM users ORDER BY uid DESC LIMIT %d,%d",
            (intval($page) - 1) * 10,
            10
        );
        $res = $DB->query($sql);
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse navbar-right" id="example-navbar-collapse">
                <ul class="nav navbar-nav">
                    <li class="active">
                        <a href="./index.php">
                            <span class="glyphicon glyphicon-th-large" aria-hidden="true"></span>用户管理</a>
                    </li>
                    <li>
                        <a href="./topic.php">
                ...
                <tbody>
                    ...
                    <?php foreach ($row as $v) : ?>
                        <tr data-uid="<?php echo $v['uid']; ?>">
                            ...
                            <td>
                                <a data-toggle="modal" data-target="#editUser" data-uid="<?php echo $v['uid']; ?>" data-nickname="<?php echo $v['nickname']; ?>" data-password="" data-sex="<?php echo $v['sex']; ?>" data-qq="<?php echo $v['qq']; ?>" data-email="<?php echo $v['email']; ?>" href="javascript:;">编辑</a>
