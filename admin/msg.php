...
if ($isLogin) {
    if (intval($uINFO['user_right']) === 1) {
        $sql = sprintf(
            "SELECT * FROM comments ORDER BY send_time DESC LIMIT %d,%d",
            (intval($page) - 1) * 10,
            10
        );
        $res = $DB->query($sql);
                    ...
                    <?php foreach ($cTmp as $v) : ?>
                        <tr data-cid="<?php echo $v['cid']; ?>">
                            ...
                            <td style="min-width: 44px">
                                <a data-toggle="modal" data-target="#reply" href="javascript:;" data-name="<?php echo $uTmp[$v['uid']]['nickname']; ?>" data-cid="<?php echo $v['cid']; ?>">回复</a>
