...
if (isset($_SERVER['REQUEST_METHOD']) && strtoupper($_SERVER['REQUEST_METHOD']) == 'POST') {
    ...
    if (get('action') === 'reg') {
        ...
        if (!$_p) {
            die(echoApiData(1, '密码至少由 字母、数字或特殊字符其中两种组成，且长度在 8~16位之间'));
        }
        ...
        $sql = "INSERT INTO users (nickname, password, qq, email, sex, summary, reg_time, reg_ip) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $DB->prepare($sql);
        $stmt->bind_param("ssssisss", $nickname,$password,$qq,$email,$sex,$summary,$time,$getip); //必须是$para传递，不能直接传值
        $stmt->execute();
...
<div class="container col-md-6 col-md-offset-3" style="padding-top: 72px;">
    ...
    <!-- Bootstrap简单form表单样式form-horizontal -->
    <form class="form-horizontal" onsubmit="return regAccount(this)">
    <!-- Bootstrap框架中的form-group，存在多个form时，添加form间隔 -->
            ...
            <div style="margin-top: 16px">
                ...
                <div class="col-xs-8">
                    <button name="regBtn" data-loading-text="注册账号中..."
                            type="submit" class="btn btn-success btn-lg btn-block">立即注册
                    </button>
                </div>
            </div>
        </div>
