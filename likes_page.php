...
$sql = sprintf("SELECT * FROM `topic_comments_replies` where topic_id=%d ORDER BY likes_count DESC LIMIT %d,%d", $topic,(intval($page) - 1) *
10, 10);
...
<div class="container col-md-8 col-md-offset-2" style="padding-top: 72px;">
    ...
    if ($res) {
        while ($tmp = $res->fetch_assoc()) {
            echo "<li class='inline-item'><a class='button' href='likes_page.php?topic={$tmp['topic_id']}'>{$tmp['name']}</a></li>";
        }
        $res->free_result();// 释放结果集
    }
    echo "</ul>";
