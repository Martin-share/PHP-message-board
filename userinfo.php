...
$admin_id = intval(get('admin_id', 0));
...
// if ($isAdminLogin){
//     if (!(intval($uINFO['admin_id']) === ($admin_id-10000))) {
//         $admin_id = intval($uINFO['admin_id']+10000);
//         echo "<script>
//             alert('横向越权？11');  
//             setTimeout(function() {
//                 window.location.href = './userinfo.php?admin_id=" . $admin_id . "';
//             }, 500);
//         </script>";
//         exit; // 终止PHP脚本的执行
//     }
// }
// else {
//     if (!(intval($uINFO['uid']) === ($uid-10000))) {
//         $uid = intval($uINFO['uid']+10000);
//         echo "<script>
//             alert('横向越权？');
//             setTimeout(function() {
//                 window.location.href = './userinfo.php?uid=" . $uid . "';
//             }, 500);
//         </script>";
//         exit; // 终止PHP脚本的执行
//     }
...
if ($uid < 10001) {
    ...
    $sql = sprintf("SELECT * FROM users WHERE uid=%d LIMIT 1", intval($uid) -
        10000);
    $res = $DB->query($sql);
    ...
    if ($res) {
        ...
        //查询关注者
        $sql = sprintf("SELECT * FROM `followed_users` WHERE following_id=%d ORDER BY follow_time DESC",
            intval($uid) - 10000);        
        $res = $DB->query($sql);//查询最新10条留言
        $follow = $res->fetch_all(MYSQLI_ASSOC);
        $res->free_result();
    }
            ...
            <?php else: ?>
                <div class="col-md-3 col-sm-12 text-center">
                    <img alt="用户头像" class="img-circle"
                         ...
                         src = "http://q2.qlogo.cn/headimg_dl?dst_uin=<?php echo $userInfo['qq']; ?>&spec=100">
                    <h3>
                    <!-- src="https://q1.qlogo.cn/g?b=qq&nk=<?php echo $userInfo['qq']; ?>&s=0" -->
                        <span><?php echo ['', '♂', '♀'][$userInfo['sex']] ?></span> <?php echo $userInfo['nickname'];
                    ?></h3>
                ...
                <div class="col-md-9 col-sm-12 text-center">
                    <table class="table table-striped table-bordered">
                        ...
                        <td><b>获赞总数</b></td>
                        <td align="center"><code><?php echo $userInfo['all_likes_count']; ?></code></td>
                        </tr>
                        </tbody>
                    </table>
...
<!-- 关注的人 -->
<?php if ((!$notFound) && ($uINFO['uid']==($uid-10000))): 
    // var_dump($uid);
    ?>
    
        <div id="followUsers" class="panel panel-info">
            <div class="panel-heading">
                <h3 class="panel-title">关注的人 <small class="text-center"></small></h3>
            </div>
            <table class="table table-hover">
                ...
                <tr>
                    <th>关注者</th>
                    <th>关注者个签</th>
                    <th style="min-width: 88px;">关注时间</th>
                ...
                <?php foreach ($follow as $row): 
                    ...
                    <tr data-cid="<?php echo $row['uid'] + 10000; ?>">
                    <!-- <a target="_blank" href="userinfo.php?uid=<?php echo $v['uid'] + 10000; ?>"> -->
                    <th scope="row"><a href="userinfo.php?uid=<?php echo $row['follower_id']+10000; ?>"><?php echo $row['nickname']; ?></a></th>
                        <!-- <th scope="row"><?php echo $row['nickname']; ?></th> -->
                        <th scope="row"><?php echo $row['summary']; ?></th>
                        <td><?php echo date("Y-m-d H:i", $row['follow_time']); ?></td>
                        <td>
                        <a href="javascript:void(0);" onclick="deleteFollow('<?php echo $row['follower_id']; ?>')">取消关注</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            ...
            <table class="table table-hover">
                ...
                <?php foreach ($comment as $row): ?>
                    <tr data-cid="<?php echo $row['cid'] + 10000; ?>">
                        ...
                        <td><?php echo join("</p><p>", explode("\n", htmlspecialchars($row['contents']))); ?></td>
